class PagesController <ApplicationController
    ...
    def search
      if params[:search].present? && params[:search].strip != ""
         session[:loveurtrip_search] = params[:search]
      end
      
      arrResult = Array.new
      
      if session[:loveurtrip_search] && session[:loveurtrip_search] != ""
         @rooms_address = Room.where(active: true).near(session[:loveurtrip_search], 5, order:'distance')
      else
